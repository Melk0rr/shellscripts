...
if [ -n "$run_sel" ] && [ "$(echo "$run_sel" | wc -l)" -eq 1 ]; then
  ...
  if [ "$repeat" = true ]; then
    ...
    while true; do
      repeat_command=$(echo -e "Repeat" | rofi -dmenu -config "${roDir}/keybind_hint.rasi" -p "[Enter] repeat; [ESC] exit") #? Needed a separate Rasi ? Dunno how to make; Maybe Something like confirmation rasi for buttons Yes and No then the -p will be the Question like Proceed? Repeat?
