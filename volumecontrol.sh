...
notify_vol() {
    ...
    [[ "${isNotify}" == true ]] && notify-send -a "t2" -r 8 -t 800 -i "${ico}" "${vol}${bar}" "${nsink}"
}
...
notify_mute() {
    ...
    if [ "${mute}" == "true" ]; then
        [[ "${isNotify}" == true ]] && notify-send -a "t2" -r 8 -t 800 -i "${icodir}/muted-${dvce}.svg" "muted" "${nsink}"
    else
        [[ "${isNotify}" == true ]] && notify-send -a "t2" -r 8 -t 800 -i "${icodir}/unmuted-${dvce}.svg" "unmuted" "${nsink}"
    fi
}
